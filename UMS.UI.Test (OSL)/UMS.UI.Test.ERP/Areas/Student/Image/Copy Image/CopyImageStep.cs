...
namespace UMS.UI.Test.ERP
{
    ...
    public class CopyImageStep
    {
        ...
        public CopyImageStep(CopyImagePage page)
        {
            _page = page;
            _output = _page.Output();
        }

        [Given("Go to Copy Image Page")]
        public void GivenGoToCopyImagePage()
        {
            _page.Output().WriteLine("AkashAkashAkash");
        ...
        public void ThenAValidationMessageWillAppear()
        {
            ...
            try
            {
                ...
                Assert.Contains("Copied Successfully", message);
            }
            catch (NoSuchElementException)
            {
                ...
                bool isValidMessage =
                    message.Contains("Blank Item Selected") ||
                    message.Contains("No Image to copy") ||
                    message.Contains("Please select at least two Program");

                Assert.True(isValidMessage, $"Unexpected validation message: {message}");
            }
        }
